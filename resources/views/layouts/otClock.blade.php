...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body onload="initClock()" class="vertical-layout vertical-menu-modern navbar-floating footer-static" data-open="click" data-menu="vertical-menu-modern" data-col="" l>
        ...
        <form action="{{ route('otClocks.insert') }}" method="POST" enctype="multipart/form-data">
              @csrf
              ...
              <!-- <input type="file" multiple name="provePhoto" class="form-control" placeholder="Photo"> -->
              <label style="font-weight:bold;text-decoration:underline">Click Here To Upload Or Capture Photo</label>
                        ...
                        <input type="file" name="provePhoto" accept="image/*" capture="camera" id="cam"/>
              </div><br>

              <input type="hidden" name="attendance_id" value="{{ $id }}">
              <div class="clockinout d-grid col-md-12 text-center mb-2" >
                 @if($type == 0)
                    <br><button class="btnclock timein active btn-primary waves-effect waves-float waves-light" name="otClock" value="0" data-type="timein">Clock In</button>
                  @else
                  <br><button class="btnclock timeout active btn-primary waves-effect waves-float waves-light" name="otClock" value="1" data-type="timeout">Clock Out</button>
                  @endif
              </div>
              </div>
              </form>
